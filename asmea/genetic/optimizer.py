...
class Optimizer():
    ...
    def get_sorted_networks1(self, networks):
        sortedNetwork = [(self.fitness(network), network) for network in networks]
        return nsga2.get_sorted_RankAndCrowdingSurvival(sortedNetwork, len(sortedNetwork))

    def get_sorted_networks2(self, networks):
        sortedNetwork = [(self.fitness2(network), network) for network in networks]
        return nsga2.get_sorted_RankAndCrowdingSurvival(sortedNetwork, len(sortedNetwork))

    def get_sorted_networks3(self, networks):
        sortedNetwork = [(self.fitness3(network), network) for network in networks]
        return nsga2.get_sorted_RankAndCrowdingSurvival(sortedNetwork, len(sortedNetwork))
   ...
   #                  )
    ...
    def fitness2(self, network):
        ...
        if self.args.objective == 'single':
            ...
            n1=k(network.params)+(1-k)(network.flops)
            return (100-network.accuracy,
                    n1
                    # network.flops,
                    # network.epoch_time
