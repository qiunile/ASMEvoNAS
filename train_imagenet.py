...
from asmea.network.model import PyramidNetworkImageNet as PyramidNetwork
...
from multadds_count import comp_multadds
...
parser.add_argument('--arch', type=str, default='RNAS', help='which architecture to use')  # 而这个是对应的hash值，并加载了预训练的模型
parser.add_argument('--grad_clip', type=float, default=5, help='gradient clipping')
parser.add_argument('--label_smooth', type=float, default=0.1, help='label smoothing')
parser.add_argument('--lr_scheduler', type=str, default='linear', help='lr scheduler, linear or cosine')
parser.add_argument('--tmp_data_dir', type=str, default='/raid/data/zhangchuang/ImageNet', help='temp data dir')
...
parser.add_argument('--lmdb', action='store_true', default=True, help='Use lmdb to load data')
...
args.save = '/raid/data/zhangchuang/code/zhangchuang/EEEA-Net/imagenet/{}-eval-{}-{}'.format(args.save, args.note,
...
def main():
    ...
    if args.pyramid == False:
        model = NetworkImageNet(args.init_channels, CLASSES, args.layers, args.auxiliary, genotype, args.drop_path_prob,
                                "FP32")
    else:
        model = PyramidNetwork(args.init_channels, CLASSES, args.layers, args.auxiliary, genotype, args.drop_path_prob,
                               "FP32", args.se, args.increment)
    ...
    mult_adds = comp_multadds(model, input_size=(3, 224, 224))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        valid_acc_top1, valid_acc_top5, valid_obj = inference.infer_imagenet(args, valid_queue, model, criterion_smooth,
                                                                             logging_mode=True)
